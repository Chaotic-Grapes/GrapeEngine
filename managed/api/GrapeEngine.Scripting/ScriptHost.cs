        ...
        try
        {
            ...
            if (scriptType == null)
            {
                ...
                foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
                {
                    ...
                    if (!asmNameStr.StartsWith("System") && 
                        ...
                        asmNameStr != "GrapeEngine.Scripting")
                    {
                        try
                        {
            ...
            if (scriptBehaviour == null)
            {
                ...
                NativeLog($"ERROR: Type identity mismatch for {typeName}" + Environment.NewLine +
                            ...
                            $"SOLUTION: Ensure EchoesBelow.dll references the same GrapeEngine.Scripting.dll that is currently loaded", LogLevel.Error);
                return 0;
            }
        ...
        try
        {
            ...
            // Register assembly resolver to find ScriptAPI in the same directory
                    ...
                    AppDomain.CurrentDomain.AssemblyResolve += (sender, args) =>
                {
                    ...
                    if (requestedName.Name == "GrapeEngine.Scripting")
                    {
            ...
            var scriptApiRef = assembly.GetReferencedAssemblies()
                .FirstOrDefault(a => a.Name == "GrapeEngine.Scripting");
