    ...
    if (ImGui::Begin("Prefab Editor", &m_editingPrefab)) {
        ...
        if (m_prefabData.contains("Components")) {
            for (auto& componentEntry : m_prefabData["Components"]) {
                ...
                if (componentType == "Transform") {
                    // [this] lets the lambda call member functions of this class (e.g. RenderFloatRow)
                    ...
                    _renderComponentSection("Transform", "Transform", data, [this](nlohmann::json& d) {
                        EditorUI::RenderFloatRow("Local Rotation", "R", d, "Rotation", 1.0f, 19.0f);
                        EditorUI::RenderVector2DRow("Local Position", d["Position"], "X", "Y", 1.0f);
                        EditorUI::RenderVector2DRow("Local Scale", d["Scale"], "X", "Y", 0.01f, 41.0f);
                        }, false);
                }
                // SPRITE RENDERER
                // Texture, color tint, flip options, sorting
                    ...
                    _renderComponentSection("Sprite Renderer", "SpriteRenderer", data, [this](nlohmann::json& d) {
                        ...
                        // Color tint picker
                        EditorUI::RenderColorProperty("Color##Sprite", d["Color"], 110.0f);

                        // Flip X/Y checkboxes
                        EditorUI::RenderCheckboxRow("Flip", d, "FlipX", "X", "FlipY", "Y", 126.0f);
                        ...
                        if (ImGui::CollapsingHeader("Additional Settings")) {
                            ...
                            EditorUI::RenderTextProperty("Sorting Layer", d, "SortingLayerName", 44.0f);

                            // Order in layer (fine-grained sorting within a layer)
                            EditorUI::RenderIntProperty("Order in Layer", d, "SortingOrder", 37.0f);
                        }
                        else {
                ...
                // RIGIDBODY 2D
                    ...
                    _renderComponentSection("Rigidbody2D", "Rigidbody2D", data, [this](nlohmann::json& d) {
                        ...
                        EditorUI::RenderFloatRow("Mass", "kg", d, "Mass", 0.1f, 82.0f);
                        EditorUI::RenderFloatRow("Linear Damping", "", d, "LinearDamping", 0.01f, 16.0f);
                        EditorUI::RenderFloatRow("Angular Damping", "", d, "AngularDamping", 0.01f, 3.0f);
                        EditorUI::RenderFloatRow("Gravity Scale", "", d, "GravityScale", 0.1f, 37.0f);
                        ...
                        if (ImGui::CollapsingHeader("Info")) {
                            ImGui::PopStyleColor(3);
                            ImGui::PopFont();
                            EditorUI::RenderVector2DRow("Linear Velocity", d["LinearVelocity"], "X", "Y", 1.0f, 11.0f);
                            EditorUI::RenderFloatRow("Angular Velocity", "", d, "AngularVelocity", 1.0f, 10.0f);
                            EditorUI::RenderFloatRow("Inertia", "", d, "Inertia", 0.1f, 95.0f);
                            EditorUI::RenderVector2DRow("Center of Mass", d["CenterOfMass"], "X", "Y", 0.1f, 7.0f);
                        }
                        else {
                            ImGui::PopStyleColor(3);
                            ImGui::PopFont();
                ...
                // CIRCLE COLLIDER 2D
                    ...
                    _renderComponentSection("CircleCollider2D", "CircleCollider2D", data, [this](nlohmann::json& d) {
                        ...
                        EditorUI::RenderVector2DRow("Offset##Circle", d["Offset"], "X", "Y", 1.0f, 83.0f);
                        EditorUI::RenderFloatRow("Radius##Circle", "px", d, "Radius", 1.0f, 71.0f);
                        EditorUI::RenderIntProperty("Layer##Circle", d, "Layer", 110.0f);
                        });
                }
                // BOX COLLIDER 2D
                else if (componentType == "BoxCollider2D") {
                    _renderComponentSection("BoxCollider2D", "BoxCollider2D", data, [this](nlohmann::json& d) {
                        ...
                        EditorUI::RenderVector2DRow("Offset##Box", d["Offset"], "X", "Y", 1.0f, 83.0f);
                        EditorUI::RenderVector2DRow("Size##Box", d["Size"], "X", "Y", 1.0f, 100.0f);
                        EditorUI::RenderIntProperty("Layer##Box", d, "Layer", 110.0f);
                        });
                }
                // LINE RENDERER
                else if (componentType == "LineRenderer") {
                    _renderComponentSection("LineRenderer", "LineRenderer", data, [this](nlohmann::json& d) {
                        EditorUI::RenderVector2DRow("Start", d["Start"], "X", "Y", 1.0f, 95.0f);
                        EditorUI::RenderVector2DRow("End", d["End"], "X", "Y", 1.0f, 103.0f);
                        EditorUI::RenderFloatRow("Thickness", "px", d, "Thickness", 0.1f, 42.0f);
                        EditorUI::RenderColorProperty("Color##Line", d["Color"], 111.0f);
                        });
                }
                // SHAPE RENDERER 2D
                else if (componentType == "ShapeRenderer2D") {
                    _renderComponentSection("ShapeRenderer2D", "ShapeRenderer2D", data, [this](nlohmann::json& d) {
                        ...
                        if (currentShape == 0) {      // Rectangle
                            EditorUI::RenderVector2DRow("Size##Rectangle", d["Size"], "X", "Y", 1.0f, 100.0f);
                        }
                        else if (currentShape == 1) { // Circle
                            EditorUI::RenderFloatRow("Radius##Circle2", "px", d, "Radius", 1.0f, 71.0f);
                        }
                            ...
                            for (size_t i = 0; i < points.size(); i++) {
                                ...
                                EditorUI::RenderVector2DRow("##Point", points[i], "X", "Y", 1.0f, 58.0f);
                        ...
                        ImGui::Text("Colors");
                        ImGui::PopFont();
                        EditorUI::RenderColorProperty("Fill Color##Shape", d["FillColor"], 84.0f);
                        EditorUI::RenderColorProperty("Outline Color##Shape", d["OutlineColor"], 47.0f);
                        EditorUI::RenderFloatRow("\" Thickness", "px", d, "OutlineThickness", 0.1f, 29.0f);
                        });
                }
            }
