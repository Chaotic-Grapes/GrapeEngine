...
namespace ECS {
            ...
            [&](Entity e, Components::AudioSource& src)
            {
                ...
                if (shouldPlay && !hasInstance) {
                    ...
                    if (handle) {
                        ...
                        // Check if fade-in is enabled
                        if (src.EnableFadeIn && src.FadeInDuration > 0.0f) {
                            _startFadeIn(e, handle, src.FadeInDuration, src.Volume);
                            LOG_DEBUG("AudioSystem: Started with fade-in (duration=" << src.FadeInDuration << "s)");
                        }
                        else {
                            LOG_DEBUG("AudioSystem: Started playback (no fade-in)");
                        }
                    }
                    else {
    ...
    void AudioSystem::_startFadeOutByHandle(Audio::PlaybackHandle handle, float duration) {
        ...
        syntheticEntity.Version = 0xFFFF; // Mark as synthetic
    ...
    void AudioSystem::OnSceneStop(World& world)
    {
        (void)world; // Suppress unused warning if needed
        m_hasStarted = false;

        if (m_activeSounds.empty()) {
            m_activeFades.clear();
            LOG_DEBUG("AudioSystem: Scene stopped (no active sounds)");
            return;
        }

        LOG_DEBUG("AudioSystem: Scene stopping - fading out " << m_activeSounds.size() << " sounds");

        // Fade out all sounds using handle-based approach (survives entity destruction)
        std::vector<std::pair<Entity, Audio::PlaybackHandle>> soundsToFade;

        // Copy the sounds first (to avoid iterator invalidation)
        for (auto& [entity, handle] : m_activeSounds) {
            soundsToFade.push_back({ entity, handle });
        }

        // Start fades
        for (auto& [entity, handle] : soundsToFade) {
            float fadeDuration = 0.5f; // Default fade duration

            // Try to get custom fade duration from component if entity still exists
            if (world.IsAlive(entity) && world.Has<Components::AudioSource>(entity)) {
            ...
            // Use handle-based fade (survives entity destruction)
            _startFadeOutByHandle(handle, fadeDuration);
        }

        // Clear the original entity-based map since we've moved to handle-based tracking
        m_activeSounds.clear();

        LOG_DEBUG("AudioSystem: Scene stop - started fade-outs (will complete during transition)");
    }
    ...
    void AudioSystem::_startFadeOut(Entity entity, float duration) {
        ...
        // Get current volume (assume 1.0 if we can't determine)
        ...
        for (auto& [entity, fade] : m_activeFades) {
            ...
            if (fade.Elapsed >= fade.Duration) {
                ...
                if (!fade.IsFadingIn) {
                    ...
                    LOG_DEBUG("AudioSystem: Fade-out complete, stopped entity " << entity.Index);
                }
