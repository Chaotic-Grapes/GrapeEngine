...
public:
    // Initializes the scene viewport and binds required editor and world dependencies
    ...
    // Releases scene viewport resources owned by this class
    ~SceneViewport() override;
    
    // Begins per-frame scene viewport processing
    void BeginFrame() override;

    // Handles in-world interaction input for scene viewport tools
    void HandleInWorldInteraction() override;

    // Renders scene viewport editor windows and controls
    void ShowEditorWindows() override;

    // Finalizes per-frame scene viewport work
    void EndFrame() override;

    // Injects tile palette integration used for paint and collision editing
    void SetTilePalette(TilePalettePanel* panel) { m_tilePalettePanel = panel; }

    // Toggles grid helper overlay visibility
    void SetGridVisible(bool v) { m_showGrid = v; }

    // Sets default dockspace id used for restore fallback
    void SetDefaultDockspaceId(ImGuiID dockspaceId) { m_defaultDockspaceId = dockspaceId; }

private:
    // Draws the full scene viewport content and overlays
